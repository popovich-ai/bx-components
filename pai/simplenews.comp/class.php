...
class PaiSimplenewsElementListComponent extends CBitrixComponent
{
	/**
	...
	protected function getResult()
	{
		...
		$iteratorYears = \Bitrix\Iblock\ElementTable::getList(array(
			'order'=>array('ID'=>'DESC'),
		...
		$years = array();
		while ($arItem = $iteratorYears->fetch()){
			$y = $arItem['DATE_CREATE']->format('Y');
			if(!in_array($y,$years)){
				$years[] = $y;
			}
		}

		$selectedYear = $this->arParams['YEAR'];
		if(!$selectedYear) $selectedYear = max($years);
		global $APPLICATION;
		$url = new Main\Web\Uri($APPLICATION->GetCurPageParam());
		$this->arResult['YEARS'] = array_map(function ($year) use ($url,$selectedYear){
			$link = $url->deleteParams(array('year'))->addParams(array('year'=>$year));
			$ar =  array(
				'YEAR'=>$year,
				'LINK'=>$link->getUri(),
			);
		...
		$filter['>=DATE_CREATE'] = new \Bitrix\Main\Type\Date("01.01.".($selectedYear));
		$filter['<=DATE_CREATE'] = new \Bitrix\Main\Type\Date("31.12.".($selectedYear));
		...
		while ($element = $iterator->GetNext())
		{
			if(intval($element['PREVIEW_PICTURE'])>0){
				$element['PREVIEW_PICTURE'] = array_merge(
					\CFile::GetFileArray(intval($element['PREVIEW_PICTURE'])),
					array('SRC'=>\CFile::GetPath(intval($element['PREVIEW_PICTURE'])))
				);
			}
